...
@IBDesignable open class PSRotaryKnob: AKView {
    ...
    func drawKnob(currentValue: CGFloat = 0,
        ...
        let knobPath: UIBezierPath = {
            ...
            case .polygon (let numberOfSides, let curvature):
                return bezierPathWithPolygonInRect(
                    ...
                    curvature: curvature,
                    startPoint: CGPoint(x: PSRotaryKnob.marginSize + knobDiameter / 2.0 + indicatorEnd.x,
                                        y: PSRotaryKnob.marginSize + knobDiameter / 2.0 + indicatorEnd.y),
                    offsetAngle: angle)
            }
        }()
        ...
        indicatorPath.move(to: CGPoint(x: PSRotaryKnob.marginSize + knobDiameter / 2.0 + indicatorStart.x,
                                       y: PSRotaryKnob.marginSize + knobDiameter / 2.0 + indicatorStart.y))
        indicatorPath.addLine(to: CGPoint(x: PSRotaryKnob.marginSize + knobDiameter / 2.0 + indicatorEnd.x,
                                          y: PSRotaryKnob.marginSize + knobDiameter / 2.0 + indicatorEnd.y))
        ...
        if isDragging {
            ...
            let bubblePath = UIBezierPath(roundedRect: bubbleRect,
                                          byRoundingCorners: .allCorners,
                                          cornerRadii: CGSize(width: PSRotaryKnob.bubbleCornerRadius,
                                                              height: PSRotaryKnob.bubbleCornerRadius))
            knobColor.setFill()
            bubblePath.fill()
    ...
    func bezierPathWithPolygonInRect(_ rect: CGRect,
        ...
        for i in 0...numberOfSides {
            ...
            if curvature == 0.0 {
                ...
                if curvature > PSRotaryKnob.maximumPolygonCurvature {
                    actualCurvature = PSRotaryKnob.maximumPolygonCurvature
                }
                if curvature < PSRotaryKnob.maximumPolygonCurvature * -1.0 {
                    actualCurvature = PSRotaryKnob.maximumPolygonCurvature * -1.0
                }
